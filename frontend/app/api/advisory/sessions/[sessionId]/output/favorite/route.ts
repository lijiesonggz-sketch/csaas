...
export async function PUT(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    const response = await fetch(
      `${readBackendUrl()}/advisory/sessions/${encodeURIComponent(params.sessionId)}/output/favorite`,
      {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          ...(typeof body?.isFavorited === 'boolean' ? { isFavorited: body.isFavorited } : {}),
        }),
        cache: 'no-store',
      }
    )
    ...
    console.error('[API /advisory/sessions/:sessionId/output/favorite PUT] Error:', error)
