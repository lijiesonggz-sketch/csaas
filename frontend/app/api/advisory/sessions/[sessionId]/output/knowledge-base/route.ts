...
export async function GET(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    const targetUrl = new URL(
      `/advisory/sessions/${encodeURIComponent(params.sessionId)}/output/knowledge-base/state`,
      readBackendUrl()
    )
    ...
    console.error('[API /advisory/sessions/:sessionId/output/knowledge-base GET] Error:', error)
...
export async function PUT(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    const response = await fetch(
      `${readBackendUrl()}/advisory/sessions/${encodeURIComponent(params.sessionId)}/output/knowledge-base`,
      {
        method: 'PUT',
        headers: {
          Authorization: authorization,
    ...
    console.error('[API /advisory/sessions/:sessionId/output/knowledge-base PUT] Error:', error)
    return NextResponse.json({ message: 'Internal server error' }, { status: 500 })
  }
}
