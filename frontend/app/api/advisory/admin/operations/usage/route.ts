...
const ALLOWED_QUERY_PARAMS = ['tenantId', 'dateFrom', 'dateTo', 'workflowType'] as const
...
function buildBackendUsageUrl(request: NextRequest) {
  ...
  return `${resolveBackendUrl()}/advisory/admin/operations/usage${query ? `?${query}` : ''}`
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    let backendUsageUrl: string
    try {
      backendUsageUrl = buildBackendUsageUrl(request)
    } catch {
      return NextResponse.json({ message: 'Duplicate filter parameter' }, { status: 400 })
    }

    const response = await fetch(backendUsageUrl, {
      method: 'GET',
      headers: {
    ...
    console.error('[API /advisory/admin/operations/usage] GET error:', error)
