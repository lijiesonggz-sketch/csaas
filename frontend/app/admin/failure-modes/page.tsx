...
export default function FailureModeAdminPage() {
  ...
  const [items, setItems] = useState<FailureModeDetail[]>([])
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [detail, setDetail] = useState<FailureModeDetail | null>(null)
  const [draft, setDraft] = useState({
    name: '',
    description: '',
    category: 'DEFINITION_ERROR' as FailureModeCategory,
  ...
  const [taxonomyTree, setTaxonomyTree] = useState<Array<{
    l1Code: string
    l1Name: string
    sortOrder: number
  ...
  const [selectedRelevance, setSelectedRelevance] = useState<(typeof RELEVANCE_OPTIONS)[number]>('PRIMARY')
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        setItems(listResult.items as FailureModeDetail[])
        setTotal(listResult.total)
        setTaxonomyTree(taxonomyResult)
        setSelectedId((current) => {
          if (current && listResult.items.some((item) => item.failureModeId === current)) {
            return current
          }
          return listResult.items[0]?.failureModeId ?? null
        })
      } catch (loadError) {
        if (!cancelled) {
          setError(errorMessage(loadError, '加载 Failure Mode 列表失败'))
        }
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    async function loadDetail() {
      try {
        ...
        if (!cancelled) {
          toast.error(errorMessage(loadError, '加载 Failure Mode 详情失败'))
        }
      } finally {
        if (!cancelled) {
  ...
  async function handleSearchControls() {
    ...
    try {
      ...
      setControlResults(result.items)
    } catch (searchError) {
        ...
        <div className="mx-auto max-w-7xl space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="rounded-sm" onClick={() => setReloadToken((current) => current + 1)}>
              ...
              <CardContent className="space-y-4 p-5">
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label>分类</Label>
                      <Select value={filters.category} onValueChange={(value) => setFilters((current) => ({ ...current, category: value as FailureModeCategory | 'all' }))}>
                    ...
                    <div className="space-y-2">
                      <Label>状态</Label>
                      <Select value={filters.status} onValueChange={(value) => setFilters((current) => ({ ...current, status: value as FailureModeStatus | 'all' }))}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {STATUS_OPTIONS.map((item) => (
                            <SelectItem key={item} value={item}>{item === 'all' ? '全部' : item}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>
                  ...
                  <div className="flex gap-2">
                    <Button className="flex-1 rounded-sm" onClick={() => { setPage(1); setAppliedFilters(filters) }}>
                ...
                <div className="flex items-center justify-between text-sm text-[#64748B]">
                  <span>总数 {total}</span>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="rounded-sm" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>
                      上一页
                    </Button>
                    <Button variant="outline" size="sm" className="rounded-sm" disabled={items.length < 20} onClick={() => setPage((current) => current + 1)}>
                      下一页
                    </Button>
                  </div>
                </div>
                ...
                {detailLoading ? (
                  ...
                  <>
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <Label>分类</Label>
                        <Select value={draft.category} onValueChange={(value) => setDraft((current) => ({ ...current, category: value as FailureModeCategory }))}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {CATEGORY_OPTIONS.map((item) => (
                              <SelectItem key={item} value={item}>{item}</SelectItem>
                      ...
                      <div className="space-y-2">
                        <Label>状态</Label>
                        <Select value={draft.status} onValueChange={(value) => setDraft((current) => ({ ...current, status: value as FailureModeStatus }))}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="rounded-sm border border-[#E2E8F0] p-4">
                      ...
                      <div className="flex flex-col gap-3 md:flex-row">
                        <Select value={selectedL2Code} onValueChange={setSelectedL2Code}>
                          <SelectTrigger className="md:flex-1"><SelectValue placeholder="选择 IT 二级分类" /></SelectTrigger>
                          <SelectContent>
                        ...
                        <Button className="rounded-sm" onClick={handleAddTaxonomyMap} disabled={!selectedL2Code || saving}>
                          <Plus className="mr-2 h-4 w-4" />
                          添加映射
                        </Button>
                      </div>
                      <div className="mt-4 space-y-2">
                        {detail.taxonomyMaps.length === 0 ? (
                          ...
                          detail.taxonomyMaps.map((map) => (
                            <div key={map.id} className="flex items-center justify-between rounded-sm border border-[#E2E8F0] px-3 py-2">
                              <div>
                                <div className="font-medium text-[#1E3A5F]">{map.l2Code}</div>
                                <div className="text-sm text-[#64748B]">{map.l2Name || '未命名分类'}</div>
                              </div>
                              <Button variant="ghost" size="icon" onClick={() => handleDeleteTaxonomyMap(map.id)}>
                    ...
                    <div className="rounded-sm border border-[#E2E8F0] p-4">
                      ...
                      <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
                        <Input value={controlKeyword} onChange={(event) => setControlKeyword(event.target.value)} placeholder="搜索 control code / control name" />
                        <Button variant="outline" className="rounded-sm" onClick={handleSearchControls} disabled={controlLoading}>
                          {controlLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Search className="mr-2 h-4 w-4" />搜索控制点</>}
                        </Button>
                      </div>
                      <div className="mt-3 max-w-[200px]">
                        <Label>关联强度</Label>
                        <Select value={selectedRelevance} onValueChange={(value) => setSelectedRelevance(value as (typeof RELEVANCE_OPTIONS)[number])}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                      ...
                      {controlResults.length > 0 && (
                        <div className="mt-3 space-y-2 rounded-sm border border-dashed border-[#CBD5E1] p-3">
                          {controlResults.map((item) => (
                            <div key={item.controlId} className="flex items-center justify-between rounded-sm border border-[#E2E8F0] px-3 py-2">
                              <div>
                                <div className="font-medium text-[#1E3A5F]">{item.controlCode} · {item.controlName}</div>
                                <div className="text-xs text-[#64748B]">{item.controlFamily} · {item.l1Code} / {item.l2Code}</div>
                              </div>
                              <Button size="sm" className="rounded-sm" onClick={() => handleAddControlMap(item.controlId)}>
                                添加为映射
                              </Button>
                            </div>
                          ))}
                      ...
                      <div className="mt-4 space-y-2">
                        {detail.controlMaps.length === 0 ? (
                          ...
                          detail.controlMaps.map((map) => (
                            <div key={map.id} className="flex items-center justify-between rounded-sm border border-[#E2E8F0] px-3 py-2">
                              <div>
                                <div className="font-medium text-[#1E3A5F]">{map.controlCode} · {map.controlName}</div>
                                <div className="text-xs text-[#64748B]">{map.relevance} · {map.maturityLevel || 'unknown'} · score {map.authoritativeScore ?? '--'}</div>
                              </div>
                              <Button variant="ghost" size="icon" onClick={() => handleDeleteControlMap(map.id)}>
                                <Trash2 className="h-4 w-4 text-rose-600" />
                              </Button>
                            </div>
                          ))
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="rounded-sm">
          <DialogHeader>
            ...
            <DialogDescription>创建新的失效模式字典项。</DialogDescription>
          </DialogHeader>
          <form className="space-y-4" onSubmit={handleCreate}>
            <div className="space-y-2">
              <Label htmlFor="failure-mode-code">编码</Label>
              <Input id="failure-mode-code" value={createForm.failureModeCode} onChange={(event) => setCreateForm((current) => ({ ...current, failureModeCode: event.target.value }))} placeholder="例如 FM-REP-003" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="failure-mode-name">名称</Label>
              <Input id="failure-mode-name" value={createForm.name} onChange={(event) => setCreateForm((current) => ({ ...current, name: event.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label>分类</Label>
              <Select value={createForm.category} onValueChange={(value) => setCreateForm((current) => ({ ...current, category: value as FailureModeCategory }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {CATEGORY_OPTIONS.map((item) => (
                    <SelectItem key={item} value={item}>{item}</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="failure-mode-description" rows={4} value={createForm.description} onChange={(event) => setCreateForm((current) => ({ ...current, description: event.target.value }))} />
            </div>
