...
const CATEGORY_CODE_PREFIX: Record<FailureModeCategory, string> = {
  DEFINITION_ERROR: 'DEF',
  MAPPING_ERROR: 'MAP',
  MISSING_CONTROL: 'MIS',
  TIMELINESS_FAILURE: 'TIM',
  INTEGRITY_FAILURE: 'INT',
  UNAUTHORIZED_ACTION: 'UNA',
  FALSIFICATION: 'FAL',
}
...
export function suggestFailureModeCode(input: {
  category: FailureModeCategory
  existingCodes: string[]
}) {
  const prefix = CATEGORY_CODE_PREFIX[input.category]
  const pattern = new RegExp(`^FM-${prefix}-(\\d{3})$`)
  ...
  return `FM-${prefix}-${String(nextSerial).padStart(3, '0')}`
}
...
export function listFailureModes(params: FailureModeListParams = {}) {
  ...
  return apiFetch(
    `/api/admin/knowledge-graph/failure-modes${queryString}`,
    NO_STORE
  ) as Promise<FailureModeListResponse>
}

export function getFailureMode(failureModeId: string) {
  return apiFetch(
    `/api/admin/knowledge-graph/failure-modes/${failureModeId}`,
    NO_STORE
  ) as Promise<FailureModeDetail>
}
...
export function deleteFailureModeTaxonomyMap(failureModeId: string, mapId: string) {
  return apiFetch(
    `/api/admin/knowledge-graph/failure-modes/${failureModeId}/taxonomy-maps/${mapId}`,
    {
      method: 'DELETE',
    }
...
export function deleteFailureModeControlMap(failureModeId: string, mapId: string) {
  return apiFetch(
    `/api/admin/knowledge-graph/failure-modes/${failureModeId}/control-maps/${mapId}`,
    {
      method: 'DELETE',
    }
  ) as Promise<{ success: true; id: string }>
...
export function getTaxonomyTree() {
  return apiFetch('/api/admin/knowledge-graph/taxonomy/tree?status=ACTIVE', NO_STORE) as Promise<
    TaxonomyTreeNode[]
  >
}
