...
export default function ProjectListShadcn({ onProjectClick }: ProjectListProps) {
  ...
  if (loading) {
    return (
      <main
        className="flex items-center justify-center min-h-[400px] bg-[#FEFDFB]"
        role="status"
        aria-label="加载中"
      >
  ...
  if (error) {
    return (
      <main
        className="max-w-[1920px] mx-auto px-6 py-6 bg-[#FEFDFB]"
        role="alert"
        aria-live="polite"
      >
    ...
    <main className="w-full px-6 py-8 bg-[#FEFDFB] min-h-screen">
      ...
      {projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
          ...
          <h2 className="text-xl font-semibold text-[#1E3A5F] mb-2 font-[var(--font-plus-jakarta)]">
            还没有任何项目
          </h2>
          <p className="text-[#94A3B8] mb-8 font-[var(--font-inter)]">
