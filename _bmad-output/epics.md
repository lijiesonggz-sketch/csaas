...
As a ?????,
I want ?????????????? crawler ? file-import ????,
So that ????? category ???????????????? RawContent ? AI ?????

**Acceptance Criteria:**

**Given** ????? tech / industry / compliance ??????????
**When** ??????
**Then** ?? category ?????? `crawler` ? `file-import`
**And** ?????????? Epic ??

**Given** category ?? `crawler`
**When** ??????
**Then** ?? BullMQ ??????
**And** ???????source?category?url?schedule?priority
**And** ???????? RawContent ?

**Given** category ?? `file-import`
**When** ????????????
**Then** ?????????????
**And** ?? `csv`?`xlsx`?`xls`?`md`?`txt`
**And** ???????????? `~$*.xlsx`?
...
**Given** RawContent ????
**When** ????
**Then** ?? BullMQ ?? `ai:analyze-content`
**And** ?????? crawler ?? file-import?????? AI ????

**Given** ????????????
**When** Epic 4 ?? `file-import` ??
**Then** ??? Story ???????
**And** ???????????????
...
**And** `contentType='penalty'`
**And** ?????????????

**Given** RawContent ????
**When** ????
**Then** ?? BullMQ ?? `ai:analyze-content`
**And** ?? Epic 2 ? AI ?????Story 2.2?????
