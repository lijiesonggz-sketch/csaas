...
export class AdvisoryOutputExportService {
  ...
  async exportSessionOutput(
    ...
    const rendered =
      format === 'markdown'
        ? {
            buffer: Buffer.from(this.renderMarkdown(output, generatedAt), 'utf8'),
  ...
  private renderMarkdown(output: AdvisoryWorkflowOutput, generatedAt: string): string {
    return [
      `# ${this.toMarkdownHeadingText(output.title)}`,
      output.summary,
      AI_LABEL,
      this.renderGenerationMetadata(output, generatedAt),
      '```jsonld',
      JSON.stringify(this.toJsonLdMetadata(output, generatedAt), null, 2),
      '```',
      ...output.sections.flatMap((section) => [
        `## ${this.toMarkdownHeadingText(section.heading)}`,
        section.contentMarkdown,
        this.renderSectionMetadata(section),
      ]),
      '',
    ]
  ...
  private renderGenerationMetadata(output: AdvisoryWorkflowOutput, generatedAt: string): string {
    const metadata = output.aiLabelMetadata ?? {}
    const lines = [
      `Generated by: ${this.readText(metadata.generator) ?? 'ThinkTank'}`,
      `Workflow: ${output.workflowKey}`,
      `Generated at: ${this.readText(metadata.generated_at) ?? generatedAt}`,
  ...
  private toJsonLdMetadata(
    ...
    return {
      ...
      '@type': 'CreativeWork',
      visible_label: AI_LABEL,
      ai_generated: true,
      machine_readable: true,
      generator: this.readText(output.aiLabelMetadata?.generator) ?? 'ThinkTank',
      workflow_key: output.workflowKey,
      source_session_id: output.sessionId,
      output_id: output.id,
