...
describe('AdvisoryOutputExportService (ATDD RED)', () => {
  ...
  test('[P0] exports the active draft as Markdown without requiring workflow completion', async () => {
    const service = createService()
    ...
    expect(markdown).toContain('[AI Generated]')
    expect(markdown).toContain('```jsonld')
    expect(markdown).toContain('"machine_readable": true')
    expect(outputRepository.createDraft).not.toHaveBeenCalled()
  })
