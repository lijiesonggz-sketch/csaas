...
import { classifyIt04CaseText, loadIt04TaxonomyMappings } from '../it04-benchmark.runner'
...
describe('Story 6.2 - Taxonomy Domain Governance (ATDD)', () => {
  it.skip('[P0][6.2-UNIT-005] should assign independent rulebook versions across IT01-IT08 and keep precedence centralized in engine', () => {
  ...
  it.skip('[P1][6.2-UNIT-006] should expose per-domain readiness metadata and cap 6.2-delivered domains at runtime-classifier-ready', () => {
    for (const l1Code of TAXONOMY_MULTIDOMAIN_ATDD_SUPPORTED_L1_CODES) {
  ...
  it.skip('[P1][6.2-INT-007] should keep the IT04 benchmark helper on the shared engine after multi-domain registry expansion', () => {
    ...
    const ruleCase = TAXONOMY_MULTIDOMAIN_ATDD_RULE_CASES.find((entry) => entry.l1Code === 'IT04')
