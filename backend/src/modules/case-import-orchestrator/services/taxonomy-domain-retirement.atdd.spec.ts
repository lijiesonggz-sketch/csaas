...
describe('Story 6.6 - Taxonomy Domain Retirement (ATDD)', () => {
  it(
    '[P0][6.6-INT-001] should refuse legacy-off execution when kill-switch drill, rollback verification, or reclassify/backfill readiness evidence is missing even if metric gate passes',
    async () => {
      const { TaxonomyDomainRetirementService } = require('./taxonomy-domain-retirement.service')
  ...
  it(
    '[P0][6.6-INT-002] should execute per-domain legacy-off, disable only that domain legacy path, and emit a retirement report without deleting audit or compatibility readers',
    async () => {
      const { TaxonomyDomainRetirementService } = require('./taxonomy-domain-retirement.service')
      ...
      expect(
        legacyPathManager.removeBenchmarkCompatibility,
      ).not.toHaveBeenCalled()
    },
  )

  it(
    '[P1][6.6-INT-003] should block physical cleanup for the first non-IT04 domain until a separate release and stable-window guard are satisfied',
    async () => {
      const { TaxonomyDomainRetirementService } = require('./taxonomy-domain-retirement.service')
