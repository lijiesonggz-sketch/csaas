...
describe('AuditLogService', () => {
  ...
  describe('logStrict', () => {
    it('should return the saved audit log when persistence succeeds', async () => {
      const logData = {
        userId: 'user-123',
        tenantId: 'tenant-456',
      ...
      const mockAuditLog = { id: 'log-strict-001', ...logData }
      repository.create.mockReturnValue(mockAuditLog as any)
      repository.save.mockResolvedValue(mockAuditLog as any)

      const result = await service.logStrict(logData)
    ...
    it('should throw when strict audit persistence fails', async () => {
      const logData = {
        userId: 'user-123',
        tenantId: 'tenant-456',
        action: AuditAction.UPDATE,
      ...
      await expect(service.logStrict(logData)).rejects.toThrow('Database error')
    })
  })
  ...
  describe('findByResource', () => {
    it('should return audit logs for a specific resource', async () => {
      ...
      const mockLogs = [{ id: 'log-001', tenantId, entityType: resource, entityId: resourceId }]
